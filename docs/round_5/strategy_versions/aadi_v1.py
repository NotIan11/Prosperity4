...
class ScaledDirectionalStrategy(DirectionalStrategy):
    """Directional strategy with a reduced effective target position.

    The exchange/product limit stays at actual_position_limit, but this strategy
    only targets +/- effective_position_limit. This directly reduces directional
    exposure without changing the rest of the strategy framework.
    """

    def __init__(
        self,
        symbol: str,
        actual_position_limit: int,
        effective_position_limit: int,
        direction: int,
    ) -> None:
        super().__init__(symbol, actual_position_limit, direction)
        self.effective_position_limit = effective_position_limit
    ...
    def run(self, state: TradingState) -> List[Order]:
        ...
        pos = self.get_position(state)
        target = self.direction * self.effective_position_limit
...
PRODUCTS = {
    ...
    "ROBOT_IRONING":   trend_round5(ScaledDirectionalStrategy("ROBOT_IRONING",   10, effective_position_limit=7, direction=-1)),  # drift −2170, reduced exposure
    ...
    # PANEL
    "PANEL_2X4": trend_round5(ScaledDirectionalStrategy("PANEL_2X4", 10, effective_position_limit=7, direction=+1)),  # drift +2354, reduced exposure

    # SLEEP_POD: POLYESTER/SUEDE/COTTON all drift well above 10,000
    "SLEEP_POD_POLYESTER":  trend_round5(ScaledDirectionalStrategy("SLEEP_POD_POLYESTER",  10, effective_position_limit=7, direction=+1)),  # drift +1970, reduced exposure
    ...
    "UV_VISOR_MAGENTA": trend_round5(ScaledDirectionalStrategy("UV_VISOR_MAGENTA", 10, effective_position_limit=5, direction=+1)),  # drift +1532, reduced exposure

    # ROBOT
    "ROBOT_MOPPING": DirectionalStrategy("ROBOT_MOPPING", 10, direction=+1),  # drift +1588
    "ROBOT_DISHES":  trend_round5(ScaledDirectionalStrategy("ROBOT_DISHES",  10, effective_position_limit=5, direction=+1)),  # drift +1200, reduced exposure
