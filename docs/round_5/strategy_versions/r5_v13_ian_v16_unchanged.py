...
class DelayedStrategy(Strategy):
    """Keep a strategy inactive until an intraday timestamp."""

    def __init__(self, strategy: Strategy, start_time: int) -> None:
        super().__init__(strategy.symbol, strategy.position_limit)
        self.strategy = strategy
        self.start_time = start_time

    def run(self, state: TradingState) -> List[Order]:
        if state.timestamp < self.start_time:
            return []
        return self.strategy.run(state)

    def save_state(self) -> dict:
        return self.strategy.save_state()
...
ROUND5_DELAYED_ENTRY_TIME = 100_000


def delay_round5(strategy: Strategy) -> DelayedStrategy:
    return DelayedStrategy(strategy, start_time=ROUND5_DELAYED_ENTRY_TIME)
...
PRODUCTS = {
    ...
    "SNACKPACK_RASPBERRY":      delay_round5(EMAMarketMaker("SNACKPACK_RASPBERRY",      10, ema_alpha=0.01, take_edge=10_000)),
    "TRANSLATOR_GRAPHITE_MIST": EMAMarketMaker("TRANSLATOR_GRAPHITE_MIST", 10, ema_alpha=0.005, take_edge=200, skew_per_lot=0.5),
    "GALAXY_SOUNDS_DARK_MATTER":      delay_round5(EMAMarketMaker("GALAXY_SOUNDS_DARK_MATTER",      10, ema_alpha=0.01, take_edge=150)),
    "GALAXY_SOUNDS_PLANETARY_RINGS":  delay_round5(EMAMarketMaker("GALAXY_SOUNDS_PLANETARY_RINGS",  10, ema_alpha=0.01, take_edge=10_000)),
    "GALAXY_SOUNDS_SOLAR_WINDS":      delay_round5(EMAMarketMaker("GALAXY_SOUNDS_SOLAR_WINDS",      10, ema_alpha=0.01, take_edge=200)),
    "MICROCHIP_CIRCLE":               delay_round5(EMAMarketMaker("MICROCHIP_CIRCLE",               10, ema_alpha=0.01, take_edge=10_000)),
    "OXYGEN_SHAKE_CHOCOLATE":         EMAMarketMaker("OXYGEN_SHAKE_CHOCOLATE",         10, ema_alpha=0.01, take_edge=150),
    "OXYGEN_SHAKE_MINT":              delay_round5(EMAMarketMaker("OXYGEN_SHAKE_MINT",              10, ema_alpha=0.01, take_edge=10_000)),
    ...
    "ROBOT_IRONING":   delay_round5(DirectionalStrategy("ROBOT_IRONING",   10, direction=-1)),  # drift −2170
    ...
    "PANEL_1X4": delay_round5(EMAMarketMaker("PANEL_1X4", 10, ema_alpha=0.01, take_edge=10_000)),  # drift −772
    "PANEL_2X2": delay_round5(EMAMarketMaker("PANEL_2X2", 10, ema_alpha=0.01, take_edge=100)),  # drift −607
    ...
    # MICROCHIP
    "MICROCHIP_SQUARE": delay_round5(DirectionalStrategy("MICROCHIP_SQUARE", 10, direction=+1)),  # drift +3633
    ...
    "GALAXY_SOUNDS_BLACK_HOLES":  delay_round5(DirectionalStrategy("GALAXY_SOUNDS_BLACK_HOLES",  10, direction=+1)),  # drift +3458
    "GALAXY_SOUNDS_SOLAR_FLAMES": DirectionalStrategy("GALAXY_SOUNDS_SOLAR_FLAMES", 10, direction=+1),  # drift  +823

    # PANEL
    "PANEL_2X4": delay_round5(DirectionalStrategy("PANEL_2X4", 10, direction=+1)),  # drift +2354

    # SLEEP_POD: POLYESTER/SUEDE/COTTON all drift well above 10,000
    "SLEEP_POD_POLYESTER":  delay_round5(DirectionalStrategy("SLEEP_POD_POLYESTER",  10, direction=+1)),  # drift +1970
    ...
    "UV_VISOR_MAGENTA": delay_round5(DirectionalStrategy("UV_VISOR_MAGENTA", 10, direction=+1)),  # drift +1532

    # ROBOT
    "ROBOT_MOPPING": DirectionalStrategy("ROBOT_MOPPING", 10, direction=+1),  # drift +1588
    "ROBOT_DISHES":  delay_round5(DirectionalStrategy("ROBOT_DISHES",  10, direction=+1)),  # drift +1200
