...
class DirectionalStrategy(Strategy):
    ...
    With stop_loss_ticks > 0: exits if mid moves stop_loss_ticks against bet
    direction. After reentry_cooldown_ts ticks (and if we have stops left),
    resets entry tracking and may re-enter. After max_stops, kill switch.
    """

    def __init__(self, symbol: str, position_limit: int, direction: int,
                 stop_loss_ticks: float = 0.0,
                 reentry_cooldown_ts: int = 100_000,
                 max_stops: int = 2) -> None:
        super().__init__(symbol, position_limit)
        self.direction = direction
        self.stop_loss_ticks = stop_loss_ticks
        self.reentry_cooldown_ts = reentry_cooldown_ts
        self.max_stops = max_stops
        self.entry_mid: Optional[float] = None
        self.stopped: bool = False
        self.stop_count: int = 0
        self.stop_ts: Optional[int] = None
    ...
    def load_state(self, data: dict) -> None:
        self.entry_mid = data.get("entry_mid")
        self.stopped = bool(data.get("stopped", False))
        self.stop_count = int(data.get("stop_count", 0))
        self.stop_ts = data.get("stop_ts")
    ...
    def run(self, state: TradingState) -> List[Order]:
        ...
        # cooldown expired and we still have stops budget → re-arm
        if (
            self.stopped
            and self.stop_count < self.max_stops
            and self.stop_ts is not None
            and state.timestamp - self.stop_ts >= self.reentry_cooldown_ts
            and pos == 0
        ):
            self.stopped = False
            self.entry_mid = None
        ...
        if self.entry_mid is None and pos != 0 and not self.stopped:
            self.entry_mid = mid

        if (
            self.stop_loss_ticks > 0
            and not self.stopped
            and self.entry_mid is not None
            and pos != 0
        ):
            adverse = (self.entry_mid - mid) if self.direction > 0 else (mid - self.entry_mid)
            if adverse >= self.stop_loss_ticks:
                self.stopped = True
                self.stop_count += 1
                self.stop_ts = state.timestamp

        if self.stopped:
            if pos > 0:
                return [Order(self.symbol, best_bid, -pos)]
            if pos < 0:
                return [Order(self.symbol, best_ask, -pos)]
            return []
...
PRODUCTS = {
    ...
    "PEBBLES_XS": trend_round5(DirectionalStrategy("PEBBLES_XS", 10, direction=-1, stop_loss_ticks=1000)),  # drift −3962
    "PEBBLES_S":  trend_round5(DirectionalStrategy("PEBBLES_S",  10, direction=-1, stop_loss_ticks=1000)),  # drift −1934
    ...
    "MICROCHIP_OVAL":      trend_round5(DirectionalStrategy("MICROCHIP_OVAL",      10, direction=-1, stop_loss_ticks=1000)),  # drift −4481
    "MICROCHIP_TRIANGLE":  trend_round5(DirectionalStrategy("MICROCHIP_TRIANGLE",  10, direction=-1, stop_loss_ticks=1000)),  # drift −2058
    ...
    "ROBOT_IRONING":   trend_round5(DirectionalStrategy("ROBOT_IRONING",   10, direction=-1, stop_loss_ticks=1000)),  # drift −2170
    "ROBOT_VACUUMING": trend_round5(DirectionalStrategy("ROBOT_VACUUMING", 10, direction=-1, stop_loss_ticks=1000)),  # drift −1725
    ...
    "TRANSLATOR_SPACE_GRAY":  trend_round5(DirectionalStrategy("TRANSLATOR_SPACE_GRAY",  10, direction=-1, stop_loss_ticks=1000)),  # drift −1571
    ...
    "PANEL_1X2": trend_round5(DirectionalStrategy("PANEL_1X2", 10, direction=-1, stop_loss_ticks=1000)),  # drift −304
    ...
    "SNACKPACK_PISTACHIO": trend_round5(DirectionalStrategy("SNACKPACK_PISTACHIO", 10, direction=-1, stop_loss_ticks=1000)),  # drift −887

    # UV_VISOR
    "UV_VISOR_AMBER":  trend_round5(DirectionalStrategy("UV_VISOR_AMBER",  10, direction=-1, stop_loss_ticks=1000)),  # drift −2870
    ...
    "PEBBLES_XL": trend_round5(DirectionalStrategy("PEBBLES_XL", 10, direction=+1, stop_loss_ticks=1000)),  # drift +6068

    # MICROCHIP
    "MICROCHIP_SQUARE": trend_round5(DirectionalStrategy("MICROCHIP_SQUARE", 10, direction=+1, stop_loss_ticks=1000)),  # drift +3633

    # OXYGEN_SHAKE
    "OXYGEN_SHAKE_GARLIC": trend_round5(DirectionalStrategy("OXYGEN_SHAKE_GARLIC", 10, direction=+1, stop_loss_ticks=1000)),  # drift +3886

    # GALAXY_SOUNDS: BLACK_HOLES has the largest upward drift in the family
    "GALAXY_SOUNDS_BLACK_HOLES":  trend_round5(DirectionalStrategy("GALAXY_SOUNDS_BLACK_HOLES",  10, direction=+1, stop_loss_ticks=1000)),  # drift +3458
    "GALAXY_SOUNDS_SOLAR_FLAMES": trend_round5(DirectionalStrategy("GALAXY_SOUNDS_SOLAR_FLAMES", 10, direction=+1, stop_loss_ticks=1000)),  # drift  +823

    # PANEL
    "PANEL_2X4": trend_round5(DirectionalStrategy("PANEL_2X4", 10, direction=+1, stop_loss_ticks=1000)),  # drift +2354

    # SLEEP_POD: POLYESTER/SUEDE/COTTON all drift well above 10,000
    "SLEEP_POD_POLYESTER":  trend_round5(DirectionalStrategy("SLEEP_POD_POLYESTER",  10, direction=+1, stop_loss_ticks=1000)),  # drift +1970
    "SLEEP_POD_SUEDE":      trend_round5(DirectionalStrategy("SLEEP_POD_SUEDE",      10, direction=+1, stop_loss_ticks=1000)),  # drift +1800
    "SLEEP_POD_COTTON":     trend_round5(DirectionalStrategy("SLEEP_POD_COTTON",     10, direction=+1, stop_loss_ticks=1000)),  # drift +1414
    "SLEEP_POD_LAMB_WOOL":  trend_round5(DirectionalStrategy("SLEEP_POD_LAMB_WOOL",  10, direction=+1, stop_loss_ticks=1000)),  # drift  +808

    # UV_VISOR
    "UV_VISOR_RED":     trend_round5(DirectionalStrategy("UV_VISOR_RED",     10, direction=+1, stop_loss_ticks=1000)),  # drift +1722
    "UV_VISOR_MAGENTA": trend_round5(DirectionalStrategy("UV_VISOR_MAGENTA", 10, direction=+1, stop_loss_ticks=1000)),  # drift +1532

    # ROBOT
    "ROBOT_MOPPING": trend_round5(DirectionalStrategy("ROBOT_MOPPING", 10, direction=+1, stop_loss_ticks=1000)),  # drift +1588
    "ROBOT_DISHES":  trend_round5(DirectionalStrategy("ROBOT_DISHES",  10, direction=+1, stop_loss_ticks=1000)),  # drift +1200

    # TRANSLATOR
    "TRANSLATOR_VOID_BLUE": trend_round5(DirectionalStrategy("TRANSLATOR_VOID_BLUE", 10, direction=+1, stop_loss_ticks=1000)),  # drift +1564
