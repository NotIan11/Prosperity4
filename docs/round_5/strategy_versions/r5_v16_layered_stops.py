...
class DirectionalStrategy(Strategy):
    ...
    With stop_loss_ticks > 0: tracks entry mid, exits and stays flat for the
    rest of the day if mid moves stop_loss_ticks against bet direction.
    """

    def __init__(self, symbol: str, position_limit: int, direction: int,
                 stop_loss_ticks: float = 0.0) -> None:
    ...
    def save_state(self) -> dict:
        return {"entry_mid": self.entry_mid, "stopped": self.stopped}
...
PRODUCTS = {
    ...
    "PEBBLES_XS": trend_round5(DirectionalStrategy("PEBBLES_XS", 10, direction=-1, stop_loss_ticks=2500)),  # drift −3962
    "PEBBLES_S":  trend_round5(DirectionalStrategy("PEBBLES_S",  10, direction=-1, stop_loss_ticks=2500)),  # drift −1934
    ...
    "MICROCHIP_OVAL":      trend_round5(DirectionalStrategy("MICROCHIP_OVAL",      10, direction=-1, stop_loss_ticks=2500)),  # drift −4481
    "MICROCHIP_TRIANGLE":  trend_round5(DirectionalStrategy("MICROCHIP_TRIANGLE",  10, direction=-1, stop_loss_ticks=2500)),  # drift −2058
    ...
    "ROBOT_IRONING":   trend_round5(DirectionalStrategy("ROBOT_IRONING",   10, direction=-1, stop_loss_ticks=2500)),  # drift −2170
    "ROBOT_VACUUMING": trend_round5(DirectionalStrategy("ROBOT_VACUUMING", 10, direction=-1, stop_loss_ticks=2500)),  # drift −1725
    ...
    "TRANSLATOR_SPACE_GRAY":  trend_round5(DirectionalStrategy("TRANSLATOR_SPACE_GRAY",  10, direction=-1, stop_loss_ticks=2500)),  # drift −1571
    ...
    "PANEL_1X2": trend_round5(DirectionalStrategy("PANEL_1X2", 10, direction=-1, stop_loss_ticks=2500)),  # drift −304
    ...
    "SNACKPACK_PISTACHIO": trend_round5(DirectionalStrategy("SNACKPACK_PISTACHIO", 10, direction=-1, stop_loss_ticks=2500)),  # drift −887

    # UV_VISOR
    "UV_VISOR_AMBER":  trend_round5(DirectionalStrategy("UV_VISOR_AMBER",  10, direction=-1, stop_loss_ticks=2500)),  # drift −2870
    ...
    # OXYGEN_SHAKE
    "OXYGEN_SHAKE_GARLIC": trend_round5(DirectionalStrategy("OXYGEN_SHAKE_GARLIC", 10, direction=+1, stop_loss_ticks=2500)),  # drift +3886

    # GALAXY_SOUNDS: BLACK_HOLES has the largest upward drift in the family
    "GALAXY_SOUNDS_BLACK_HOLES":  trend_round5(DirectionalStrategy("GALAXY_SOUNDS_BLACK_HOLES",  10, direction=+1, stop_loss_ticks=2500)),  # drift +3458
    "GALAXY_SOUNDS_SOLAR_FLAMES": trend_round5(DirectionalStrategy("GALAXY_SOUNDS_SOLAR_FLAMES", 10, direction=+1, stop_loss_ticks=2500)),  # drift  +823

    # PANEL
    "PANEL_2X4": trend_round5(DirectionalStrategy("PANEL_2X4", 10, direction=+1, stop_loss_ticks=2500)),  # drift +2354

    # SLEEP_POD: POLYESTER/SUEDE/COTTON all drift well above 10,000
    "SLEEP_POD_POLYESTER":  trend_round5(DirectionalStrategy("SLEEP_POD_POLYESTER",  10, direction=+1, stop_loss_ticks=2500)),  # drift +1970
    "SLEEP_POD_SUEDE":      trend_round5(DirectionalStrategy("SLEEP_POD_SUEDE",      10, direction=+1, stop_loss_ticks=2500)),  # drift +1800
    "SLEEP_POD_COTTON":     trend_round5(DirectionalStrategy("SLEEP_POD_COTTON",     10, direction=+1, stop_loss_ticks=2500)),  # drift +1414
    "SLEEP_POD_LAMB_WOOL":  trend_round5(DirectionalStrategy("SLEEP_POD_LAMB_WOOL",  10, direction=+1, stop_loss_ticks=2500)),  # drift  +808

    # UV_VISOR
    "UV_VISOR_RED":     trend_round5(DirectionalStrategy("UV_VISOR_RED",     10, direction=+1, stop_loss_ticks=2500)),  # drift +1722
    "UV_VISOR_MAGENTA": trend_round5(DirectionalStrategy("UV_VISOR_MAGENTA", 10, direction=+1, stop_loss_ticks=2500)),  # drift +1532

    # ROBOT
    "ROBOT_MOPPING": trend_round5(DirectionalStrategy("ROBOT_MOPPING", 10, direction=+1, stop_loss_ticks=2500)),  # drift +1588
    "ROBOT_DISHES":  trend_round5(DirectionalStrategy("ROBOT_DISHES",  10, direction=+1, stop_loss_ticks=2500)),  # drift +1200

    # TRANSLATOR
    "TRANSLATOR_VOID_BLUE": trend_round5(DirectionalStrategy("TRANSLATOR_VOID_BLUE", 10, direction=+1, stop_loss_ticks=2500)),  # drift +1564
