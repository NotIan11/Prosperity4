...
class OsmiumStrategy(Strategy):
    ...
    def run(self, state: TradingState) -> List[Order]:
        ...
        # Wall mid as dynamic reference for taking
        wall_mid = None
        ...
        if passive_buy_cap > 0:
            ...
            wall_buy = bid_wall + buy_wall_offset if bid_wall is not None else FAIR_VALUE - self.spread
            if wall_buy < buy_price:
                orders.append(Order(self.symbol, wall_buy, passive_buy_cap))
        if passive_sell_cap > 0:
            ...
            wall_sell = ask_wall - sell_wall_offset if ask_wall is not None else FAIR_VALUE + self.spread
            if wall_sell > sell_price:
                orders.append(Order(self.symbol, wall_sell, -passive_sell_cap))
