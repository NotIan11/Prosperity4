...
v10: Voucher delta-1 taker on VFE deviation, with live regression deltas
    and R² filtering so caps adapt by strike/regime instead of using static
    training-day delta assumptions.
"""
...
class VevOptionTaker:
    ...
    deviates from rolling FV by >= ENTRY_THR, take position in the voucher
    in the reversion direction. Cap is sized from live OLS delta and disabled
    when the strike's rolling R² is too weak for the current regime."""
    ...
    def __init__(self, symbol: str, position_limit: int) -> None:
        ...
        self.voucher_history: deque[float] = deque(maxlen=1000)
        self.entry_underlying: float | None = None
        self.cached_delta: float = 0.5
        self.cached_r2: float = 0.0
    ...
    def _update_delta_and_r2(self) -> None:
        self.tick_count += 1
        if self.tick_count % 50 != 0:
            return

        n = min(len(self.und_history), len(self.voucher_history))
        if n < 200:
            return

        und_values = list(self.und_history)[-n:]
        voucher_values = list(self.voucher_history)[-n:]
        mean_s = sum(und_values) / n
        mean_v = sum(voucher_values) / n

        var_s = sum((s - mean_s) * (s - mean_s) for s in und_values)
        if var_s <= 0:
            return

        cov_vs = sum((v - mean_v) * (s - mean_s) for v, s in zip(voucher_values, und_values))
        delta = cov_vs / var_s

        intercept = mean_v - delta * mean_s
        ss_res = sum(
            (v - (intercept + delta * s)) * (v - (intercept + delta * s))
            for v, s in zip(voucher_values, und_values)
        )
        ss_tot = sum((v - mean_v) * (v - mean_v) for v in voucher_values)
        r2 = 0.0 if ss_tot <= 0 else 1.0 - ss_res / ss_tot

        self.cached_delta = delta
        self.cached_r2 = max(0.0, min(1.0, r2))

    def get_dynamic_cap(self, vfe_equiv_target: int = 160) -> int:
        if self.cached_r2 < 0.5:
            return 0
        cap = int(round(vfe_equiv_target / max(self.cached_delta, 0.1)))
        return min(cap, POSITION_LIMITS[self.symbol])
    ...
    def run(self, state: TradingState) -> list[Order]:
        ...
        if (depth is None or not depth.buy_orders or not depth.sell_orders
            ...
            return []
        pos = state.position.get(self.symbol, 0)
        voucher_mid = (max(depth.buy_orders) + min(depth.sell_orders)) / 2.0
        und_mid = (max(und.buy_orders) + min(und.sell_orders)) / 2.0
        self.und_history.append(und_mid)
        self.voucher_history.append(voucher_mid)
        self._update_delta_and_r2()
        fv = self._rolling_fv()
        dev = und_mid - fv
        orders: list[Order] = []
        ...
        if self.entry_underlying is not None and pos != 0:
            ...
            if adverse > self.STOP_LOSS_TICKS:
                ...
                return orders
        buys = sells = 0
        dynamic_cap = self.get_dynamic_cap()
        if dynamic_cap <= 0:
            return orders

        if dev < -self.ENTRY_THR and pos < dynamic_cap:
            for ask_price in sorted(depth.sell_orders):
                rem = dynamic_cap - pos - buys
                if rem <= 0:
                    break
                qty = min(-depth.sell_orders[ask_price], rem)
                if qty > 0:
        ...
        elif dev > self.ENTRY_THR and pos > -dynamic_cap:
            for bid_price in sorted(depth.buy_orders, reverse=True):
                rem = dynamic_cap + pos - sells
                if rem <= 0:
                    break
                qty = min(depth.buy_orders[bid_price], rem)
                if qty > 0:
...
class Trader:
    def __init__(self) -> None:
        self.strategies = {
            ...
            # v10: adaptive voucher takers (live delta/R² on VFE deviation)
            "VEV_4000": VevOptionTaker("VEV_4000", POSITION_LIMITS["VEV_4000"]),
            "VEV_4500": VevOptionTaker("VEV_4500", POSITION_LIMITS["VEV_4500"]),
            "VEV_5000": VevOptionTaker("VEV_5000", POSITION_LIMITS["VEV_5000"]),
            "VEV_5100": VevOptionTaker("VEV_5100", POSITION_LIMITS["VEV_5100"]),
            "VEV_5200": VevOptionTaker("VEV_5200", POSITION_LIMITS["VEV_5200"]),
            "VEV_5300": VevOptionTaker("VEV_5300", POSITION_LIMITS["VEV_5300"]),
            "VEV_5400": VevOptionTaker("VEV_5400", POSITION_LIMITS["VEV_5400"]),
            "VEV_5500": VevOptionTaker("VEV_5500", POSITION_LIMITS["VEV_5500"]),
        }
