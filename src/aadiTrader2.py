...
class HydrogelStrategy(Strategy):
    ...
    # Kalman filter: fuses HYDROGEL mid + VEV_EXTRACT*ratio as two observations
    KF_Q: float = 0.2      # process noise
    KF_R1: float = 150.0   # HYDROGEL mid observation noise
    KF_R2: float = 75.0    # VEV correlation observation noise
    VEV_RATIO: float = 1.905  # HYDROGEL / VEV_EXTRACT cointegration ratio
    VEV_SYMBOL: str = "VELVETFRUIT_EXTRACT"
    ...
    def __init__(self, symbol: str, position_limit: int) -> None:
        super().__init__(symbol, position_limit)
        self.kf_x: Optional[float] = None
        self.kf_p: float = 1000.0

    def save_state(self) -> dict:
        return {"kf_x": self.kf_x, "kf_p": self.kf_p}

    def load_state(self, data: dict) -> None:
        self.kf_x = data.get("kf_x")
        self.kf_p = data.get("kf_p", 1000.0)
    ...
    def run(self, state: TradingState) -> List[Order]:
        ...
        # Update 1: HYDROGEL mid
        k1 = self.kf_p / (self.kf_p + self.KF_R1)
        self.kf_x += k1 * (mid - self.kf_x)
        self.kf_p *= (1.0 - k1)

        # Update 2: VEV_EXTRACT * ratio (if available)
        vev_depth = state.order_depths.get(self.VEV_SYMBOL)
        if vev_depth and vev_depth.buy_orders and vev_depth.sell_orders:
            ...
            z2 = vev_mid * self.VEV_RATIO
            k2 = self.kf_p / (self.kf_p + self.KF_R2)
            self.kf_x += k2 * (z2 - self.kf_x)
            self.kf_p *= (1.0 - k2)
...
class VevOptionStrategy(Strategy):
    ...
    Separately, we post passive bids in VEV_5200/5300/5400 at any VFE level,
    targeting Mark 22's recurring bid-side sells. VEV_5400 falls back to bidding
    at the current best bid when spread=1 (bid+1 would cross the spread).
    """
    ...
    # VEV_5400: BS fair > market bid by ~2 ticks, making passive bids highly
    # attractive even at spread=1 (falls back to best_bid rather than skipping).
    MARK22_PASSIVE_BID_STRIKES = {"VEV_5200", "VEV_5300", "VEV_5400"}

    EXIT_UNDERLYING_THR: float = 5_250.0
    MAX_EXIT_PER_TICK: int = 25
    MAX_PASSIVE_FAIR_PREMIUM: float = 5.0
    PASSIVE_BID_CAP = {
        "VEV_5200": 160,
        "VEV_5300": 280,
        "VEV_5400": 150,
    ...
    def directional_reversion_orders(
        ...
        # Anchor stop-loss for positions built purely via passive bids (no
        # directional entry ever set). Pin entry to current S so the 40-tick
        # stop still protects against large moves from here.
        if pos != 0 and self.entry_underlying is None:
    ...
    def mark22_passive_bid(
        ...
        if bid_price >= best_ask:
            # Spread=1: bid+1 would cross. Fall back to best_bid so we compete
            # at the existing bid level. Worthwhile when fair > best_bid (clear
            # pricing edge), as is the case for VEV_5400 (~2 ticks of edge).
            bid_price = best_bid
            if fair - bid_price < 1.0:
                return []
