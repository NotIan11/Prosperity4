...
class VevOptionStrategy(Strategy):
    ...
    MARK22_PASSIVE_BID_STRIKES = {"VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"}
    ...
    def directional_reversion_orders(
        ...
        sold = 0
        dev = S - self.FV
        entry_thr = self.DIRECTIONAL_ENTRY_THR.get(self.symbol, self.ENTRY_THR)

        if dev < -entry_thr and pos < self.position_limit:
            for ask_price in sorted(depth.sell_orders.keys()):
        ...
        elif dev > entry_thr and pos > -self.position_limit:
            for bid_price in sorted(depth.buy_orders.keys(), reverse=True):
